...
export function compilePattern(
  ...
  const cells: PatternCell[][] = Array.from({ length: options.height }, (_, y) =>
    Array.from({ length: options.width }, (_, x) => {
      const center = cellCenterToPosition(x, y, options.bbox, options.width, options.height);
      let fill = fillStyles.ground;

      for (const polygon of polygons) {
        if (pointInPolygon(center, polygon.feature.coordinates)) {
          fill = polygon.style;
        }
      }
  ...
  return {
    ...
    backstitches,
    markers,
    legend: buildLegend(cells, backstitches, markers),
  };
}
