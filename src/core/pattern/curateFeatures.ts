...
function roadEndpointSnapRadius(detail: number): number {
  return detail >= 2 ? 3 : 2;
}
...
function roadCandidateConnectionRadius(candidate: LineCandidate, detail: number): number {
  const baseRadius = roadEndpointSnapRadius(detail);

  if (routeReference(candidate.feature) && candidate.kind === 'primaryRoad') {
...
function snapSelectedRoadEndpointsToNetwork(
  ...
  const startTarget = nearestOtherSelectedCell(startEndpoint, selectedCellCounts, ownCellCounts, radius);
  if (startTarget) {
  ...
  const endTarget = nearestOtherSelectedCell(endEndpoint, selectedCellCounts, ownCellCounts, radius);
  if (endTarget) {
...
export function curateFeatures(
  ...
  for (const line of lines) {
    ...
    const useSourceGeometry = isRoadLike(kind);
  ...
  for (const candidate of collapsedLineCandidates) {
    const isSelectedRoad = selectedRoadIds.has(candidate.id);
